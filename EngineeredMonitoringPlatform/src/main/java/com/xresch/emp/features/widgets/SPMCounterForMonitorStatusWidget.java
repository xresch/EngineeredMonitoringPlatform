...
public class SPMCounterForMonitorStatusWidget extends WidgetDefinition {

	private static Logger logger = CFWLog.getLogger(SPMCounterForMonitorStatusWidget.class.getName());
	@Override
	public String getWidgetType() {return "emp_spmcounterformonitorstatus";}
						...
						.setDescription("{!cfw_widget_spm_counternames_desc!}")	
							...
							public AutocompleteResult getAutocompleteData(HttpServletRequest request, String searchValue) {
								...
								if(!Strings.isNullOrEmpty(monitors) && !monitors.equals("{}")) {
									...
									return SPMEnvironmentManagement.autocompleteCountersForMonitor(Integer.parseInt(environment), firstID, searchValue, this.getMaxResults());
								}else {
	...
	public void fetchData(JSONResponse response, JsonObject settings) { 
		...
		JsonObject countersObject = counterElement.getAsJsonObject();
		if(countersObject.size() == 0) {
			return;
		}
		...
		for(Entry<String, JsonElement> entry : countersObject.entrySet()) {
			
			String measureName = entry.getKey().trim();
			ResultSet result = db.preparedExecuteQuerySilent(
					CFW.Files.readPackageResource(FeatureEMPWidgets.RESOURCE_PACKAGE, "emp_widget_spmcounterformonitorstatus.sql"),
					monitorID,
					measureName);
	...
	public ArrayList<FileDefinition> getJavascriptFiles() {
		...
		FileDefinition js = new FileDefinition(HandlingType.JAR_RESOURCE, FeatureEMPWidgets.RESOURCE_PACKAGE, "emp_widget_spmcounterformonitorstatus.js");
		array.add(js);
		return array;
	}
