SELECT * FROM ( 
	...
	AH_STATUS AS STATUS_CODE,
	  (SELECT CASE
	    WHEN (SELECT CASE WHEN AH_STATUS > 0 THEN AH_STATUS ELSE EH_STATUS END FROM DUAL) <= 1700 THEN 'RUNNING'
	    WHEN (SELECT CASE WHEN AH_STATUS > 0 THEN AH_STATUS ELSE EH_STATUS END FROM DUAL) BETWEEN 1700 AND 1799 THEN 'WAITING'
