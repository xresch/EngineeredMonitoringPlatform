...
public class WidgetTimersForMonitor extends WidgetDefinition {
	...
	public void fetchData(HttpServletRequest request, JSONResponse response, JsonObject settings) { 
		...
		for(Entry<String, JsonElement> entry : timersObject.entrySet()) {
			...
			ResultSet result = db.preparedExecuteQuerySilent(
					CFW.Files.readPackageResource(FeatureSPM.RESOURCE_PACKAGE, "emp_widget_spmtimersformonitor.sql"),
					monitorID,
					measureName);
	...
	public ArrayList<FileDefinition> getJavascriptFiles() {
		...
		FileDefinition js = new FileDefinition(HandlingType.JAR_RESOURCE, FeatureSPM.RESOURCE_PACKAGE, "emp_widget_spmtimersformonitor.js");
		array.add(js);
		return array;
	}
