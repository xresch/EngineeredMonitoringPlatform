...
public class WidgetCounterForMonitorStatus extends WidgetDefinition {
	...
	private static Logger logger = CFWLog.getLogger(WidgetCounterForMonitorStatus.class.getName());
	@Override
						...
						.setDescription("{!cfw_widget_spm_counternames_desc!}")	
							...
							public AutocompleteResult getAutocompleteData(HttpServletRequest request, String searchValue) {
								...
								if(!Strings.isNullOrEmpty(monitors) && !monitors.equals("{}")) {
									...
									return EnvironmentManagerSPM.autocompleteCountersForMonitor(Integer.parseInt(environment), firstID, searchValue, this.getMaxResults());
								}else {
	...
	public void fetchData(HttpServletRequest request, JSONResponse response, JsonObject settings) { 
		...
		for(Entry<String, JsonElement> entry : countersObject.entrySet()) {
			...
			ResultSet result = db.preparedExecuteQuerySilent(
					CFW.Files.readPackageResource(FeatureSPM.RESOURCE_PACKAGE, "emp_widget_spmcounterformonitorstatus.sql"),
					monitorID,
					measureName);
			...
			try {
				if(result != null && result.next()) {
					...
					nameResult = db.preparedExecuteQuerySilent(
							CFW.Files.readPackageResource(FeatureSPM.RESOURCE_PACKAGE, "emp_widget_spmmonitordetails.sql"),
							monitorID);
	...
	public ArrayList<FileDefinition> getJavascriptFiles() {
		...
		FileDefinition js = new FileDefinition(HandlingType.JAR_RESOURCE, FeatureSPM.RESOURCE_PACKAGE, "emp_widget_spmcounterformonitorstatus.js");
		array.add(js);
		return array;
	}
