...
public class SPMCounterForProjectStatusWidget extends WidgetDefinition {

	private static Logger logger = CFWLog.getLogger(SPMCounterForProjectStatusWidget.class.getName());
	@Override
	public String getWidgetType() {return "emp_spmcounterforprojectstatus";}
						...
						.setDescription("{!cfw_widget_spm_counternames_desc!}")	
							...
							public AutocompleteResult getAutocompleteData(HttpServletRequest request, String searchValue) {
								...
								if(!Strings.isNullOrEmpty(projects) && !projects.equals("{}")) {
									...
									return SPMEnvironmentManagement.autocompleteCountersForProject(Integer.parseInt(environment), firstID, searchValue, this.getMaxResults());
								}else {
	...
	public void fetchData(HttpServletRequest request, JSONResponse response, JsonObject settings) { 
		...
		for(Entry<String, JsonElement> entry : countersObject.entrySet()) {
			...
			ResultSet result = db.preparedExecuteQuerySilent(
					CFW.Files.readPackageResource(FeatureEMPWidgets.RESOURCE_PACKAGE, "emp_widget_spmcounterforprojectstatus.sql"),
					projectID,
					measureName);
	...
	public ArrayList<FileDefinition> getJavascriptFiles() {
		...
		FileDefinition js = new FileDefinition(HandlingType.JAR_RESOURCE, FeatureEMPWidgets.RESOURCE_PACKAGE, "emp_widget_spmcounterforprojectstatus.js");
		array.add(js);
		return array;
	}
